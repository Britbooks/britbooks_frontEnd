...
import { generateTransactionId } from '../../lib/utils/pdfGenerator.js';
...
export const getRefundsByStatus = async (status) => {
    try {
        ...
        const refunds = wallets.flatMap(wallet => wallet.refundRequests.filter(refund => refund.status === status));
