...
import { loginUser, registerUser, verifyLogin, verifyRegister } from '../services/auth';
...
function reducer(state: AuthState, action: Action): AuthState {
  switch (action.type) {
    ...
    case 'SET_PENDING':
      return { ...state, pendingToken: action.token, pendingFlow: action.flow, loading: false };
    case 'LOGOUT':
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function login(email: string, password: string) {
    ...
    try {
      ...
      dispatch({ type: 'SET_PENDING', token: data.token, flow: 'login' });
  ...
  return (
    <AuthContext.Provider value={{ ...state, login, register, verifyOtp, logout, clearError }}>
      {children}
