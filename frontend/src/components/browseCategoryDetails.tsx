...
const BrowseCategoryDetail = () => {
  ...
  // Pre-fetched shelf data — loaded in parallel with the book
  const [shelfLoading, setShelfLoading]   = useState(true);
  const [youMayAlso,   setYouMayAlso]     = useState<Book[]>([]);
  const [moreLike,     setMoreLike]       = useState<Book[]>([]);

  const inWishlist = book ? isInWishlist(book.id) : false;
  const previousCategory = (location.state as { category?: string })?.category || "Browse";

  /* fetch book + shelves in parallel */
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    ...
    // If the category hint is available from navigation state, fire shelf fetches
    // immediately alongside the book detail fetch — eliminates the waterfall.
    const hintCat = (location.state as { category?: string })?.category;
    const validHint = !!(hintCat && hintCat !== "Browse");

    const earlyShelfPromise = validHint
      ? Promise.all([
          fetchBooks({ page: 1, limit: 12, category: hintCat, sort: "purchases", order: "desc" }),
          fetchBooks({ page: 1, limit: 12, category: hintCat, sort: "listedAt",  order: "desc" }),
        ]).catch(() => null)
      : null;
    ...
    (async () => {
      try {
        ...
        const bookId = String(d._id || d.id);
        ...
        const filter = (list: Book[]) =>
          (Array.isArray(list) ? list : []).filter(b => b?.id && String(b.id) !== bookId);

        // Reuse the early shelf promise when the hint matches the book's real category.
        // If the hint was wrong or missing, fetch now (still benefits from fetchBooks cache).
        const shelfResults =
          earlyShelfPromise && validHint && hintCat === category
            ? await earlyShelfPromise
            : await Promise.all([
                fetchBooks({ page: 1, limit: 12, category, sort: "purchases", order: "desc" }),
                fetchBooks({ page: 1, limit: 12, category, sort: "listedAt",  order: "desc" }),
              ]).catch(() => null);

        if (!cancelled && shelfResults) {
          const [r1, r2] = shelfResults;
          setYouMayAlso(filter(r1?.listings ?? []));
          setMoreLike(filter(r2?.listings ?? []));
        }
      } catch {
        if (!cancelled) { setError("Failed to load book details."); toast.error("Book not found"); setLoading(false); }
      } finally {
